...
test('https://api.themoviedb.org/3/movie/now_playing?api_key=6913ffc952f25e36b31a905375c2bdad')
async function test(apiUrl) {
    const options = {
    method: 'GET',
    headers: {
        accept: 'application/json',
        Authorization: 'Bearer eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiI2OTEzZmZjOTUyZjI1ZTM2YjMxYTkwNTM3NWMyYmRhZCIsIm5iZiI6MTc0ODAxODI0NC41MTksInN1YiI6IjY4MzBhNDQ0MTNiOTFhMzdjYTJiNjY4OSIsInNjb3BlcyI6WyJhcGlfcmVhZCJdLCJ2ZXJzaW9uIjoxfQ.kFhZ_DDoVxhUL3e-qqhlAWQ-V7CAQYKVRrZjuCtArNI'
}
    };
    ...
    // }else {
    // function hoverIn(){
    //     $('.layer h2').addClass('animate__animated animate__backInDown')
    //     console.log('hello');
    //     hoverd = true
    // }
    // function hoverOut(){
    ...
    for(let x = 0; x < arrData.length; x++) {
        ...
        <div data-aos="zoom-in-up" class="col-md-6 col-lg-4 movie text-white">
            <div class="image rounded-3 overflow-hidden position-relative">
                ...
                <img src="https://image.tmdb.org/t/p/w500${arrData[x].poster_path}" class="w-100 ayhaga d-block rounded-3 " alt="">
                ...
                <div class="total ps-3">
                    ...
                    <p class=" pt-2">${arrData[x].overview.split(' ',  50).join(' ')}...</p>
...
$('.Trending').on('click', ()=> {
    test('https://api.themoviedb.org/3/trending/all/day?api_key=eba8b9a7199efdcb0ca1f96879b83c44')
    closeSideBar()
} ) 
...
$('#searchInput input').on('input', function() {
    if(this.value != '') {
        test(`https://api.themoviedb.org/3/search/movie?query=${this.value}&include_adult=false&language=en-US&page=1`)
        closeSideBar()
    }
    if(this.value == '') {
    test(`https://api.themoviedb.org/3/movie/now_playing?api_key=6913ffc952f25e36b31a905375c2bdad`)
